...
void paymentMenu(const string& nama) {
    ...
    cout << "Total Belanja: Rp" << fixed << setprecision(2) << totalAmount << endl;

    cout << "Masukkan tujuan pengiriman (0-9): ";
    cin >> tujuan;

    double ongkir = 0.0;
    if (dist[tujuan] != INT_MAX) {
        ongkir = dist[tujuan] * biayaPerKm;
        cout << "Jarak ke tujuan: " << dist[tujuan] << " km\n";
    ...
    if (amountPaid < grandTotal) {
        ...
        cout << "\nPembayaran untuk " << nama << " sebesar Rp" << fixed << setprecision(2) << grandTotal
             << " berhasil.\n";
